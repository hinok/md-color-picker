        ...
        .directive('mdColorPicker', ['$timeout', 'mdColorPickerHistory', function ($timeout, colorHistory) {
            ...
            return {
                ...
                compile: function (element, attrs) {
                    ...
                    attrs.colorModes = attrs.colorModes !== undefined ? attrs.colorModes : 'hex,rgb,hsl';
                }
            };
        }])
                ...
                controller: ["$scope", "$element", "$attrs", function ($scope, $element, $attrs) {
                    ...
                    $scope.setMaterialColor = function (event, family) {
                        ...
                        materialPreview = $compile([
                            '<div class="md-color-picker-material-colors" layout="column" >',
                    ...
                    $scope.hasColorMode = function hasRGB(colorMode) {
                        console.log('$scope.colorModes', $scope.colorModes, 'looking for:', colorMode, 'result:', $scope.colorModes.indexOf(colorMode) >= 0);
                        return $scope.colorModes.indexOf(colorMode) >= 0;
                    };
        ...
        .factory('$mdColorPicker', ['$q', '$mdDialog', 'mdColorPickerHistory', function ($q, $mdDialog, colorHistory) {
            return {
                show: function (options) {
                    ...
                    if (options.colorModes === undefined)
                        options.colorModes = 'hex,rgb,hsl';

                    console.log('$mdColorPicker.show()', 'options.colorModes', options.colorModes);

                    var dialog = $mdDialog.show({
                        template: '' +
                        '<md-dialog class="md-color-picker-dialog">' +
                        '	<div md-color-picker-dialog value="value" default="{{defaultValue}}" random="{{random}}" ok="ok" color-modes="{{colorModes}}"></div>' +
                        ...
                        controller: ['$scope', 'value', 'defaultValue', 'random', 'colorModes', function ($scope, value, defaultValue, random, colorModes) {
                            console.log('$mdDialog.show()', 'value', value);
                            console.log('$mdDialog.show()', 'colorModes', colorModes);
